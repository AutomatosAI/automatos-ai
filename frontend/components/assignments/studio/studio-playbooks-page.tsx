...
/**
 * Studio Playbooks library — CD round 4 standalone Playbooks page.
 *
 * Featured strip on top (MktCard-shaped picks from the same data),
 * then the full library as a grid of PlaybookCards or a flat table.
 * Scope tabs (All / Mine / Workspace / Imported) layer over a search
 * box and sort menu.
 *
 * "Mine" / "Workspace" / "Imported" use the playbook fields when
 * available; missing fields fall through to "All".
 */
...
export function StudioPlaybooksPage() {
  ...
  const { data, isLoading } = useWorkflowPlaybooks({ limit: 100 })
    ...
    <div className="cc-page">
      ...
      <nav className="cc-tabs" aria-label="Assignments sections">
        <button type="button" className="cc-tab active" aria-current="page">
          <span>Playbooks</span>
          {all.length > 0 && <span className="cc-tab-ct">{all.length}</span>}
        </button>
        <button
          type="button"
          className="cc-tab"
