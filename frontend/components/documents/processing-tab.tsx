...
export function ProcessingTab() {
  ...
  const loadLiveStatus = async () => {
    try {
      const data = await apiClient.getProcessingLiveStatus()
      setLiveJobs(data?.active_jobs || [])
    } catch (err) {
      console.error('Error loading live status:', err)
      setLiveJobs([]) // Clear live jobs on error
    }
  }

  const handleReprocessAll = async () => {
    try {
      await apiClient.reprocessAllDocuments()
    ...
    <div className="space-y-6">
          ...
          {pipelineData.processing_stages.map((stage, index) => (
            <div key={stage.stage} className="flex items-center justify-between p-4 bg-secondary/30 rounded-lg">
              ...
              <div className="text-right">
                <p className="text-sm font-medium">{stage.success_rate.toFixed(1)}%</p>
      ...
      >
        ...
        <div className="space-y-3">
          {pipelineData.recent_activity.length > 0 ? (
