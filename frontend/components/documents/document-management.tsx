...
export function DocumentManagement() {
  ...
  const { data: documents = [], isLoading: loading, error, refetch } = useDocuments()
  ...
  useEffect(() => {
    if (documents && Array.isArray(documents)) {
      ...
      const processedDocs = documents.filter(d => (d.status || '').toLowerCase() === 'completed' || (d.status || '').toLowerCase() === 'processed').length
      ...
      setStats([
        {
          label: 'Total Documents',
          value: documentStats?.total_documents?.toString() || totalDocs.toString(),
        ...
        {
          label: 'Processed',
          value: documentStats?.processed_documents?.toString() || processedDocs.toString(),
        ...
        {
          label: 'Storage Used',
          value: documentStats?.storage_used || sizeDisplay,
        ...
        {
          label: 'Vector Chunks',
          value: documentStats?.total_chunks?.toString() || documents.reduce((sum, d)=> sum + (d.chunk_count || 0), 0).toString(),
          change: '',
          icon: Database,
          color: 'text-purple-400'
        }
  ...
  const handleFileUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        // React Query will auto-refresh after mutation success
      }, 1000)
  ...
  const handleDelete = async (documentId: number) => {
    ...
    try {
      ...
      // React Query will auto-refresh and show success toast
    } catch (error) {
