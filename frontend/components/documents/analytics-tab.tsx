...
export function AnalyticsTab() {
  ...
  const loadAnalyticsData = async () => {
    try {
      ...
      // No fake data - show real data or nothing
      setSearchPatterns(patterns || null)
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {analyticsData.status_breakdown.map((status, index) => (
                <div key={status.status} className="p-4 bg-secondary/30 rounded-lg text-center">
                  ...
                  <p className="text-xs text-primary">{status.percentage.toFixed(1)}%</p>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <TabsContent value="insights" className="space-y-6">
          ...
          <motion.div
            ...
            transition={{ duration: 0.5, delay: 0.1 }}
          >
            <h3 className="text-lg font-semibold mb-4">Upload Trends (Last 7 Days)</h3>
