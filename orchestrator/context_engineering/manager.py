...
class EnhancedContextManager:
    ...
    - Learning from past execution patterns
    - Semantic code search and documentation retrieval
    - Advanced context constraints and optimization
    ...
    def __init__(self):
        ...
        # Advanced context management components
        try:
            from .context.constraints import ContextConstraintsManager, ContextConstraints
            from .context.hierarchy import HierarchicalMemoryManager, MemoryLevel
            from .context.compression import ContextCompressionEngine
            from .context.optimization import ContextOptimizationEngine
            
            # Initialize advanced context management
            self.constraints_manager = ContextConstraintsManager()
            self.hierarchy_manager = HierarchicalMemoryManager()
            ...
            logger.info("Advanced context management features enabled")
            
        except ImportError as e:
            logger.warning(f"Advanced context features not available: {e}")
            self.advanced_features_enabled = False
            self.constraints_manager = None
    ...
    async def advanced_rag_retrieve(
        ...
        """Advanced RAG retrieval with intelligent context management"""
        
        try:
            logger.info(f"Advanced RAG retrieval for session {session_id}, query: {query[:100]}...")
            
            # Use advanced features if available
            if self.advanced_features_enabled:
                # Validate constraints
                context_data = {"query": query, "session_id": session_id}
                valid, message = await self.constraints_manager.validate_context_addition(
                    session_id, context_data
                )
                
                if not valid:
                    logger.warning(f"Context validation failed: {message}")
                    return {"error": message, "context": ""}
                
                # Optimize context access
                optimized_query, optimization_info = await self.optimization_engine.optimize_context_access(
                    session_id, "rag_query", {"query": query}
                )
                
                # Search hierarchical memory first
                memory_results = await self.hierarchy_manager.retrieve_memory(
                    session_id, query, max_results
                )
                
                if memory_results:
                    logger.info(f"Found {len(memory_results)} results in hierarchical memory")
            
            # Fallback to standard enhanced RAG
            standard_results = await self.enhanced_rag_retrieve(
                query, include_code_similarity, project_context, max_results
            )
            
            if self.advanced_features_enabled:
                # Compress results if requested
                if use_compression and standard_results.get("context"):
                    compression_result = await self.compression_engine.compress_context(
                        {"context": standard_results["context"]},
                        method="semantic",
                        target_ratio=0.7
                    )
                    
                    standard_results["context"] = compression_result.compressed_data.get("context", "")
                    standard_results["compression_info"] = {
                ...
                if standard_results.get("context"):
                    await self.hierarchy_manager.store_memory(
                        session_id,
                        {"query": query, "results": standard_results},
                        importance
                    )
                
                # Update usage metrics
                await self.constraints_manager.update_usage_metrics(
                    session_id, {"rag_results": standard_results}
                )
                
                # Add advanced metadata
                standard_results["advanced_features"] = {
                    "optimization": optimization_info if 'optimization_info' in locals() else None,
            ...
            logger.error(f"Advanced RAG retrieval failed: {str(e)}")
