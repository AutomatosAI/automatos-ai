...
def init_db():
    ...
    with engine.begin() as conn:
        ...
        # PRD-209 carried kb_types + knowledge_items to this path but not the three
        # multimodal tables the Knowledge Bases page, the agents' table/formula
        # tools and the ingester's table extraction read and write. Same DDL as
        # alembic/versions/kb_multimodal_tables.py (a test holds them together).
        conn.execute(_raw_sql("""
        ...
        """.replace("__VISUAL_EMBEDDING__", "visual_embedding vector(512)," if has_vector else "")))
        for stmt in (
            ...
            conn.execute(_raw_sql(stmt))
        if has_vector:
            conn.execute(_raw_sql("CREATE INDEX IF NOT EXISTS idx_kb_images_visual_embedding ON kb_images USING ivfflat (visual_embedding vector_cosine_ops) WITH (lists = 100)"))
        # The nine reference rows production carries; the Multimodal tab lists types from here.
        conn.execute(_raw_sql("""
            INSERT INTO kb_types (type_name, display_name, description, icon, processor_class, storage_strategy,
