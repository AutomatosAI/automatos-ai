...
Two write-side fixes:
  1. The 30s durable-store health probe used to write one ``heartbeat_results`` row per
...
Pure — the probe is driven with a mocked durable store + a fake scheduler; the daily-summary
...
def _install_durable(monkeypatch, healthy: bool):
    """Point the probe at a fake durable store with the given health.
    ...
    fake_client.health = AsyncMock(return_value={"healthy": healthy})
    fake_unified = MagicMock()
    fake_unified._durable = fake_client
...
async def test_probe_emits_only_on_state_change(monkeypatch):
    ...
    fake_client = _install_durable(monkeypatch, healthy=True)
    ...
    await svc._durable_memory_probe_tick()
    ...
    # Tick 3 — durable store goes down → transition → one more emit.
    fake_client.health = AsyncMock(return_value={"healthy": False})
    await svc._durable_memory_probe_tick()
...
@pytest.mark.asyncio
async def test_probe_pings_store_every_tick(monkeypatch):
    # The health round-trip is the loud failure surface and must run on EVERY
    # tick, independent of the (transition-gated) primitive emit.
    svc = HeartbeatService()
    svc._scheduler = _fake_scheduler_with_ws("ws-1")
    fake_client = _install_durable(monkeypatch, healthy=True)
    monkeypatch.setattr(
    ...
    await svc._durable_memory_probe_tick()
    await svc._durable_memory_probe_tick()
    assert fake_client.health.await_count == 2
