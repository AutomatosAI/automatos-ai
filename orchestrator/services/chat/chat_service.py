"""
Chat Service - Thin Orchestrator
=================================

Main chat service orchestrating:
- Database operations (ChatService)
- Streaming responses (StreamingChatService)

Delegates specialized work to:
- PromptAnalyzer: Message conversion and tool intent
- MemoryInjector: Memory retrieval and storage
- ToolRouter: Tool execution and formatting
...
from services.chat.prompt_analyzer import get_prompt_analyzer
from services.chat.memory_injector import get_memory_injector
from services.chat.tool_router import get_tool_router, get_chatbot_tools, execute_tool
from services.chat.streaming_handler import get_streaming_handler

logger = logging.getLogger(__name__)


# =============================================================================
# TOOL EXPORTS (backward compatibility)
# =============================================================================

def get_chat_tools() -> List[Dict[str, Any]]:
    """Get chatbot tools from ToolRegistry."""
...
class StreamingChatService:
    ...
    Thin orchestrator delegating to specialized modules.
    """
    ...
    async def stream_response(
        ...
        try:
            ...
            if hasattr(llm_manager, 'generate_response_stream'):
                async for chunk in llm_manager.generate_response_stream(messages=llm_messages, tools=tools or []):
