...
Handles:
- Getting tools from ToolRegistry
- Executing tools via UnifiedToolExecutor
...
def get_chatbot_tools() -> List[Dict[str, Any]]:
    ...
    SINGLE SOURCE OF TRUTH for chatbot tools.
    """
    ...
    try:
        ...
        logger.debug(f"Loaded {len(openai_tools)} tools from registry")
        return openai_tools
    except Exception as e:
        logger.warning(f"Error loading tools from registry: {e}")
        return _get_fallback_tools()


def _get_fallback_tools() -> List[Dict[str, Any]]:
    """Fallback tools if registry unavailable."""
    logger.warning("Using minimal fallback tools")
    return [
        {
            "type": "function",
            "function": {
        ...
        },
        {
            ...
            "function": {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {"query": {"type": "string", "description": "Plain English query"}},
                    "required": ["query"]
                }
            }
        }
