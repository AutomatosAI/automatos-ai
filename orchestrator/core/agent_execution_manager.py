        ...
        if self.context_manager and hasattr(self, 'shared_context') and self.shared_context:
            try:
                ...
                for key, value in context_data.items():
                    ...
                    if key.startswith("subtask_") and key.endswith("_result"):
                        ...
                        if isinstance(value, dict):
                            result_preview = value.get('response', 'No response')[:3000]  # Increased from 500 to 3000
                            result_length = len(value.get('response', ''))
                        else:
                            result_preview = str(value)[:3000]  # Increased from 500 to 3000
                            result_length = len(str(value))
                        
                        previous_results.append(f"\n**Previous Task Result ({key}):**\n{result_preview}\n")
                ...
                if previous_results:
                    ...
                    context_section = "\n\n" + "="*80 + "\n"
                    context_section += "🔥 CRITICAL: CONTEXT FROM PREVIOUS TASKS - YOU MUST USE THIS INFORMATION!\n"
                    context_section += "="*80 + "\n"
                    context_section += "The following results were generated by other agents in your team.\n"
                    context_section += "You MUST incorporate this information into your response.\n"
                    context_section += "Do NOT ignore or summarize this - use the actual details, code, and examples provided.\n"
                    context_section += "="*80 + "\n\n"
