...
from core.models.core import Agent, AgentCatalogTemplate, Skill, User as UserModel, WorkflowTemplate as WorkflowRecipe
...
# ===================================================================
# Agent Catalog Templates (PRD-120: Skills Marketplace)
# ===================================================================

class AgentCatalogTemplateOut(BaseModel):
    """Agent template from the catalog for marketplace browsing."""
    id: int
    slug: str
    name: str
    category: str
...
class AgentCatalogTemplateDetail(AgentCatalogTemplateOut):
    """Extended template detail including persona text."""
...
class AgentCatalogCategoryOut(BaseModel):
    """Category summary with agent count."""
    category: str
    count: int
    icon: Optional[str] = None


class AgentCatalogDeployResponse(BaseModel):
    """Response from deploying an agent template."""
    success: bool
    agent_id: int
    agent_name: str
    message: str
...
# ===================================================================
# GET /api/marketplace/agents — Browse agent catalog templates
# ===================================================================

@router.get("/agents", response_model=List[AgentCatalogTemplateOut])
async def list_agent_templates(
    ...
    try:
        query = db.query(AgentCatalogTemplate).filter(
            AgentCatalogTemplate.is_active == True
        )

        if category:
            query = query.filter(AgentCatalogTemplate.category == category)

        if search:
            search_term = f"%{search}%"
            query = query.filter(or_(
                AgentCatalogTemplate.name.ilike(search_term),
                AgentCatalogTemplate.description.ilike(search_term),
            ))

        query = query.order_by(AgentCatalogTemplate.category, AgentCatalogTemplate.name)
        templates = query.offset(offset).limit(limit).all()

        return [
            AgentCatalogTemplateOut(
                id=t.id,
                slug=t.slug,
                name=t.name,
                category=t.category,
                ...
                recommended_tools=t.recommended_tools or [],
                tags=t.tags or [],
                icon=t.icon,
                tier=t.tier,
            )
        ...
        logger.error(f"Error listing agent catalog templates: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")


# ===================================================================
# GET /api/marketplace/agents/categories — Category list with counts
# ===================================================================

@router.get("/agents/categories", response_model=List[AgentCatalogCategoryOut])
async def list_agent_categories(
    ctx: RequestContext = Depends(get_request_context_hybrid),
    db: Session = Depends(get_db),
):
    """Return list of categories with template counts and icons."""
    try:
        rows = (
            db.query(
                AgentCatalogTemplate.category,
                func.count(AgentCatalogTemplate.id).label("count"),
                func.min(AgentCatalogTemplate.icon).label("icon"),
            )
            .filter(AgentCatalogTemplate.is_active == True)
            .group_by(AgentCatalogTemplate.category)
            .order_by(AgentCatalogTemplate.category)
            .all()
        )

        return [
            AgentCatalogCategoryOut(category=row.category, count=row.count, icon=row.icon)
            for row in rows
        ]

    except Exception as e:
        logger.error(f"Error listing agent catalog categories: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")


# ===================================================================
# GET /api/marketplace/agents/{slug} — Single template detail
# ===================================================================

@router.get("/agents/{slug}", response_model=AgentCatalogTemplateDetail)
async def get_agent_template(
    slug: str,
    ctx: RequestContext = Depends(get_request_context_hybrid),
    db: Session = Depends(get_db),
):
    """Get full detail for a single agent catalog template by slug."""
    try:
        template = db.query(AgentCatalogTemplate).filter(
            AgentCatalogTemplate.slug == slug,
            AgentCatalogTemplate.is_active == True,
        ).first()

        if not template:
            raise HTTPException(status_code=404, detail=f"Agent template '{slug}' not found")

        return AgentCatalogTemplateDetail(
            id=template.id,
            slug=template.slug,
            name=template.name,
            ...
            skill_slug=template.skill_slug,
            recommended_model=template.recommended_model,
            recommended_tools=template.recommended_tools or [],
        ...
        logger.error(f"Error fetching agent template '{slug}': {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")


# ===================================================================
# POST /api/marketplace/agents/{slug}/deploy — Create agent in workspace
# ===================================================================

@router.post("/agents/{slug}/deploy", response_model=AgentCatalogDeployResponse)
async def deploy_agent_template(
    slug: str,
    ctx: RequestContext = Depends(get_request_context_hybrid),
    db: Session = Depends(get_db),
):
    """Deploy an agent catalog template into the user's workspace.

    Creates a new Agent with the template's persona, model config, and
    optionally links a matching skill and Composio tool assignments.
    """
    try:
        template = db.query(AgentCatalogTemplate).filter(
            AgentCatalogTemplate.slug == slug,
        ...
        # Check for duplicate agent name in workspace
        ...
        # Build model_config from recommended_model
        ...
        agent = Agent(
            ...
            owner_type="workspace",
            owner_id=str(ctx.workspace_id),
            created_by="marketplace-deploy",
            custom_persona_prompt=template.persona,
            use_custom_persona=bool(template.persona),
        )
        db.add(agent)
        db.flush()

        # Link skill if a matching one exists (by skill_slug or name)
        if template.skill_slug:
            skill = db.query(Skill).filter(
                Skill.name.ilike(template.skill_slug.replace("-", " ").replace("_", " ")),
                Skill.is_active == True,
            ).first()
            # Also try package_slug match
            if not skill:
                skill = db.query(Skill).filter(
                    Skill.package_slug == template.skill_slug,
                    Skill.is_active == True,
                ).first()
            if skill:
                agent.skills.append(skill)

        # Assign recommended Composio tools
        if template.recommended_tools:
            from core.models.composio_cache import AgentAppAssignment
            for tool_name in template.recommended_tools:
                # Only assign Composio-style tools (uppercase app names like GITHUB, SLACK)
                if tool_name and tool_name == tool_name.upper() and not tool_name.startswith("workspace_"):
        ...
        logger.info(
            f"Deployed agent template '{slug}' as agent {agent.id} "
            f"('{agent_name}') in workspace {ctx.workspace_id}"
        )

        return AgentCatalogDeployResponse(
            success=True,
            agent_id=agent.id,
            agent_name=agent_name,
            message=f"Agent '{agent_name}' created from template '{template.name}'",
        )

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error deploying agent template '{slug}': {e}", exc_info=True)
        db.rollback()
        raise HTTPException(status_code=500, detail="Internal server error")
