...
async def vote_message(
    ...
    MUST be registered before ``PATCH /{chat_id}`` -- routes match in
    declaration order, so with this below the param route
    ``PATCH /api/chat/vote`` resolved as ``chat_id="vote"`` and every vote
    404'd (PRD-205 S8; the exact PRD-220 ``/search`` failure mode).
    """
    chat_service = ChatService(db)
    user_id = get_user_id(db, ctx)
...
async def get_available_agents(
    ...
    MUST be registered before ``GET /{chat_id}`` -- routes match in
    declaration order, so with this below the param route
    ``GET /api/chat/agents`` resolved as ``chat_id="agents"`` and 404'd
    (PRD-205 S8; the exact PRD-220 ``/search`` failure mode).
    """
...
# NOTE (PRD-205 S8): ``PATCH /vote`` and ``GET /agents`` were declared here,
# BELOW their ``/{chat_id}`` param routes, which dead-shadowed both
# (``chat_id="vote"`` / ``chat_id="agents"``). They now live above
# ``GET /{chat_id}`` next to ``/search`` -- same paths, same handlers, no
# manifest delta; declaration order is the only change.
