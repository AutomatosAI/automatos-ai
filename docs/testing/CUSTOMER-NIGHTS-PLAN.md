...
| 5 | **Missions + shared field memory** | three missions of increasing size; explicit staffing instructions ("the words to the writer"); the second mission needs facts the first one learned | plan quality, assignment obeys the owner, approval flow, synthesis, deliverables per mission; **shared field memory (PRD-166, Qdrant `field_memory`, on since 18 Sep 21:06): facts written per mission, promoted to durable, recalled by a *different* agent in the next mission, recall hit rate, wrong recalls** — the follow-up numbers for Gerard's field-memory paper | night 1 found assignment ignored the instruction; memory was off until 21:06 |
