...
The ledger, per night: tickets by status and by runtime (cli Claude / cli Codex / API model), ticket durations p50/p95 per runtime, cost by request_type and by agent, Auto turns with tokens per turn and tools used, tool calls per agent and failure rate, asks raised / answered / expired, deliverables and reports counted and graded, retrieval hits, routing decisions. This is the data for the November analytics.
...
| 5 | **Missions** | three missions of increasing size; explicit staffing instructions ("the words to the writer") | plan quality, assignment obeys the owner, approval flow, synthesis, deliverables per mission | night 1 found assignment ignored the instruction |
...
| 8 | **Auto's brain — baseline** | 60–100 varied asks to Auto across every capability, in one evening | which tools it chose, wasted calls, tokens per turn (night 1: ~37k), latency, wrong routes; the semantic tool graph before/after (`tool_routing_*` rows, `routing_decisions`); the offline 77-query eval as the fixed yardstick | `python -m scripts.eval.tool_routing.run_eval` beside the live night |
| 9 | **Auto's brain — Jev** | the same evening, PRD-248 seam on: shadow first (rows only, no behaviour change), then `classifier_mode=live` / `tool_rerank_mode=live` | tool-selection accuracy vs night 8, latency per turn, cost per turn, disagreements listed; `evals.operating_graph_uplift --ranker jev` offline | the PoC branch `feat+prd-248-decision-seam` merged into the test checkout; TypeSafe/OpenRouter key |
...
| 11 | **Hybrid runtimes** | the night-1 evening with a mixed team: Claude sessions, Codex sessions, API agents on OpenAI, DeepSeek, Kimi via OpenRouter — identical briefs to each | quality (judge), cost, time per runtime on the same brief; the "some agents perform better" table | the models present in the registry; budget |
...
5. The night-1 fixes as PRs: agents' knowledge access; hold cards with intent + full command + `comm` allowlisted; Command Centre tiles; Agent Reports panel; the Answer button; the SLA-as-calendar entry.
...
4. When PRD-248's branch joins the test checkout (before night 9).
