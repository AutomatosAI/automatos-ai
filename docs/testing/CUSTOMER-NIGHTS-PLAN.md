...
The ledger, per night (window = `night.status` STARTED→FINISHED, UTC; `llm_usage.created_at` is naive UTC): tickets by status and by runtime (cli Claude / cli Codex / API model), ticket durations p50/p95 per runtime, cost by request_type and by agent, Auto turns with tokens per turn and tools used, tool calls per agent and failure rate, asks raised / answered / expired, deliverables and reports counted and graded, retrieval hits, routing decisions, and the decision lane (`request_type='decision'`: count, cost, latency p50/p95 by provider) with the `complexity_assessor` count beside it. Model-drift checks whitelist `request_type in ('decision','embedding')`. This is the data for the November analytics.
