...
- Auto's per-workspace persona is seeded by `core/seeds/seed_auto_agent.py`: `_default_persona()` = the friendly base voice (`_FRIENDLY_FALLBACK`) + the identity-level `MANAGER_DOCTRINE_BLOCK`, written to the Auto row's `custom_persona_prompt`. Auto is additionally attached the always-on `platform-management` skill (`core/seeds/platform-management-skill.md`, the procedure-level doctrine home). The `auto-cto-custom-soul.txt` file feeds the GLOBAL, admin-only CTO agent (`core/seeds/seed_cto_agent.py`) — **not** the per-workspace Auto agent. Personas live in the DB (house rule: no file hacks for DB data — the seed code is the source, rows are the runtime).
...
- **Component A — seed update + backfill.** Identity-level doctrine into `MANAGER_DOCTRINE_BLOCK` (`seed_auto_agent.py`, appended to `_default_persona()` — friendly base + doctrine, not a rewrite) and lane/contract mechanics into `platform-management-skill.md` (procedure-level, the always-on skill). Auto rows are per-workspace (`auto-{workspace_id}`): the idempotent seed-sync backfill `sync_auto_personas` (invoked once per deploy from `main.py` boot, no migration) updates every existing Auto row whose `custom_persona_prompt` still hash-matches a shipped default to the new `_default_persona()`, and skips + reports customized rows (no compat shims). (The CTO-voiced `auto-cto-custom-soul.txt`, enriched with the same doctrine in this wave, is the CTO agent's identity via `seed_cto_agent.py` — not Auto's.)
