...
- **D2 · One Command Centre.** The shell is the Command Centre at every desktop width, whatever the theme: its `cc-*` rules are scoped to the shell's own root (`:is(.studio, .cc-page)`, the calendar's pattern), the Activity tab is already the merged Feed + History stream, and the legacy page's period selector is kept in the shell and drives the stats, the Summary read and the Activity stream. The bell's deep-links then work for everyone — **this closes the Questions-tab report.** Because D6 keeps the classic tree below 1024 px through the Studio pass, `ActivityPage` and `BoardView` survive **only as that sub-1024 rendering** and are deleted in the mobile pass (PRD-245), not in Wave 1 — Gerard's sequencing, 09-17.
