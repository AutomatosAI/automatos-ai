...
**S-host · The host is always on, and always current — S (BUILT 2026-09-07, owner: "how do we make sure hosts are always running for each agent?").** One host per machine serves every `runtime: cli` agent; `--install` registers it as a login service (launchd / systemd --user) via `make cli-host-install`, KeepAlive on non-zero exit, log in the state dir. Drift → drain → restart: the host fingerprints its own modules and the heartbeat answer carries `host_contract` + `expected_host_version`; on either changing it stops claiming, lets sessions finish, exits 75. `make up` nudges a running host (`SIGHUP` via `host.pid`). `GET /api/v1/cli-hosts/health` (online, last seen, waiting CLI tickets) drives a board banner; a CLI ticket parked with no host says so on the ticket (`NO_HOST_REASON`, cleared on claim). Not built: a push notification on the online→offline transition, and Auto's ASSIGN warning when the proposed agent's host is offline.
