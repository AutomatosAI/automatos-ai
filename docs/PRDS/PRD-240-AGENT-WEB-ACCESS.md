...
| OpenAI direct | Responses API `{"type": "platform_web_search"}` | $10 / 1k calls | Our client is Chat Completions only (`openai_client.py:120`). |
...
- **D1 · Web access is a platform capability, not a marketplace app.** `platform_web_fetch` and `platform_web_search` are platform actions in the ActionRegistry (the same 3-file pattern as every `platform_*` action). Every agent has them; nothing to connect, nothing to assign; discoverable through `platform_find_tools` / `platform_execute` at near-zero standing token cost. `http_request` stays the internal self-test tool it is — its allowlist and `verify=False` are untouched.
- **D2 · Fetch is native and keyless — and resolve-and-pin.** `httpx` + TLS verification on + the user's denylist + a byte cap + HTML→text, with no key at all. The private-range check (always on, not configurable) is done on **every address the name resolves to**, and the request is then sent to **that checked address** with the hostname as `Host` and SNI — so a DNS answer that changes between the check and the connection cannot move the request into a blocked range. Redirects are followed **by hand**: each hop is resolved and checked before any request is sent to it (httpx's own `follow_redirects` would have fetched the whole chain first), capped at 5 hops.
...
- FR-2 Private, loopback, link-local and reserved ranges are refused by `platform_web_fetch` and filtered from `platform_web_search` results in every configuration; the denylist adds to that, never replaces it.
- FR-3 `platform_web_fetch` verifies TLS against the hostname, connects only to an address it has checked, checks every redirect hop **before** requesting it (max 5), caps the body at `WEB_FETCH_MAX_BYTES` while streaming, and returns text — never raw HTML — for HTML pages.
- FR-4 `platform_web_search` returns `[{title, url, snippet}]` from the first available backend in `openrouter → composio → searxng` order, or `{available:false, reason, options}` naming the three ways to enable it. It never raises into the agent loop.
- FR-5 When the agent's route is OpenRouter and `WEB_ACCESS` is on, the `openrouter:platform_web_search` server tool is attached with `max_uses`, `max_results` and the denylist as `excluded_domains` — except on a turn that forces a specific tool (`tool_choice: required`), where a web search must not be able to stand in for the forced call; citations reach the persisted reply as a Sources list. The `platform_web_search` action marks any link the model produced without searching as `unverified`.
...
- A fresh local install with an NVIDIA key and a Composio key: the same, through `platform_web_search` → Composio.
- A fresh local install with no keys: `platform_web_fetch` of a public URL returns page text; `platform_web_search` returns the honest "three options" result.
- `curl http://postgres:5432` / `http://host.docker.internal` / `http://169.254.169.254` through `platform_web_fetch` are refused in every configuration.
...
- Whether `platform_web_fetch` should also accept PDFs (Anthropic's server fetch does; the RAG pipeline already parses PDFs, so it is a small extension later).
