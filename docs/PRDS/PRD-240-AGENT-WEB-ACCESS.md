...
- FR-3 `web_fetch` verifies TLS against the hostname, connects only to an address it has checked, checks every redirect hop **before** requesting it (max 5), caps the body at `WEB_FETCH_MAX_BYTES` while streaming, and returns text — never raw HTML — for HTML pages.
