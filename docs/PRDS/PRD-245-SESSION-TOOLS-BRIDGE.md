...
- **D1 · Transport: an MCP endpoint inside the backend, on loopback.** `POST /api/v1/session-tools/mcp`, streamable HTTP, served by the official `mcp` Python SDK mounted in FastAPI. The backend already listens on 127.0.0.1:8000 for the host's claim, events and result. No stdio process per session; a stdio shim is built only if a served CLI cannot speak HTTP MCP.
...
**Files:** `api/session_tools.py` (new), `main.py`/router registration, `config.py`, `reports/route-manifest.json`, the config-surface report, `requirements.txt`; `tests/test_prd245_session_tools_endpoint.py` (fake executor).
**Test:** unauthenticated → 401; a finished ticket's token → 401; `tools/list` is byte-stable for one agent; `update_ticket` with another `task_id` is forced to the ticket's own id; `done` refused; the cap refuses call 201 with a plain message; each call reaches `execute_tool` with the ticket's agent and workspace.
