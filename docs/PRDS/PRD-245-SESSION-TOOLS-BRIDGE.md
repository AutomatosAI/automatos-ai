...
`apply_result` classifies denials: `hold` (reason carries "no answer from the operator" or "denied by the operator"), `read_outside`, `unknown_tool`, `prompt` (stage `PermissionRequest`). `force_review = any(kind in ("hold", "other"))` — an unplaceable refusal fails closed; **as built** a fifth kind, `refused`, covers the gate's outright refusals and does NOT force review. The classification rides `runtime_ref.permission_denials[*].kind` and the report's "Refused tool calls" groups by kind.
...
`claim_for_host` mints `session_token = secrets.token_urlsafe(32)`, stores `sha256` on `runtime_ref.session_token_sha256` (no migration; JSONB), returns the token, `session_tools_path` (**as built** — a PATH, so the host joins its own backend address) and `session_tools: [names]` in the claim payload. `resolve_session_token(db, token)` looks up `in_progress` tickets **with a live lease** by the hash and returns `(task, agent)`; `apply_result`, cancel and the sweeper's requeue clear the hash.
**Files:** `services/cli_host_service.py`; `api/cli_hosts.py` (payload shape); `tests/test_prd245_session_token_realdb.py`.
...
**Files:** `services/session_tools.py` (the tool + its runner), `services/cli_host_service.py`, `api/approval_grants.py`; `tests/test_prd245_session_ask_resume.py`.
...
**Files:** `services/session_tools.py`; `tests/test_prd245_session_composio.py`.
...
The stack is based on `main`. The ten PRD-244 PRs of 2026-09-17 merged into the long-running `studio` branch, not `main`, so `main` is unchanged — and the files this stack touches do not overlap the 128 that `studio` touches, so it merges into either without conflict.
