...
| 1 | **Cache hit** | <5ms | Free | Same intent pattern seen before, cached decision |
...
After Tier 3 resolves, the decision is cached (keyed by normalized intent + source) so subsequent identical requests hit Tier 1.
...
- [ ] In-memory cache (dict) with optional Redis backend for multi-instance deployments
- [ ] Cache key: `hash(normalized_content_embedding + source_type)` — use semantic similarity, not exact string match
- [ ] Cache TTL: 24 hours default, configurable
- [ ] `record_correction(request_id, correct_agent_id)` method — when user overrides after auto-routing, store the correction
...
- **FR-3:** LLM routing decisions MUST be cached. Cache key uses semantic similarity, not exact string.
...
| `orchestrator/core/routing/cache.py` | `RoutingCache` — in-memory + Redis caching with TTL |
