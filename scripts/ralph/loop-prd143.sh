...
# Ralph Wiggum Build Loop (Claude) — PRD-143 Rev 2 (Operator + Obs Lock)
# Usage:
#   ./loop-prd143.sh           # Build mode (default, runs until RALPH_COMPLETE)
#   ./loop-prd143.sh 12        # Max 12 iterations (safety cap)
#
# Branch lock: ralph/prd-143-operator-obs-lock (run from the
#   automatos-ai-prd143 worktree). Agent scope: S1-S17 (all stories).
#   S17 is the ONLY story allowed to push + open the PR; merge is Gerard's.
#   Human follow-through: obs-manifest sign-off (Open Q1/Q2), prod seed
#   apply (S12), PR review + merge. See PROMPT_build_prd143.md.
...
PROMPT_FILE="scripts/ralph/PROMPT_build_prd143.md"
...
EXPECTED_BRANCH="ralph/prd-143-operator-obs-lock"
CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")
if [[ "$CURRENT_BRANCH" != "$EXPECTED_BRANCH" ]]; then
  echo -e "${RED}Error: on branch '$CURRENT_BRANCH', expected '$EXPECTED_BRANCH'.${NC}"
  echo -e "${YELLOW}Run from the automatos-ai-prd143 worktree (git worktree add ... -b $EXPECTED_BRANCH).${NC}"
  exit 1
fi
...
echo -e "${GREEN}Ralph loop: BUILD mode — PRD-143 Rev 2 Operator + Obs Lock (17 stories; S17 pushes the PR; merge + manifest sign-off are human)${NC}"
...
while true; do
  ...
  if [[ "$RESULT_MSG" =~ RALPH_COMPLETE ]]; then
    ...
    echo -e "${GREEN}PRD-143 Rev 2 done — PR open for Gerard. Human follow-through: review + merge the PR, obs-manifest sign-off (Open Q1/Q2), prod seed apply (S12).${NC}"
    break
  fi
