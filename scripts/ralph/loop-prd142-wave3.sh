...
# Ralph Wiggum Build Loop (Claude) — PRD-142 Wave 3 (Primitive Hardening)
# Usage:
#   ./loop-prd142-wave3.sh           # Build mode (default, runs until RALPH_COMPLETE)
#   ./loop-prd142-wave3.sh 12        # Max 12 iterations (safety cap)
#
# Branch lock: ralph/prd-142-wave3-primitive-hardening (create + checkout before running)
# Agent scope: backend hardening stories only. Human-only: W3-S3 (FE tile),
#   W3-S12 deletes + FE-repoint, W3-S14 (prod migration). See PROMPT_build_prd142-wave3.md.
...
PROMPT_FILE="scripts/ralph/PROMPT_build_prd142-wave3.md"
...
EXPECTED_BRANCH="ralph/prd-142-wave3-primitive-hardening"
CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")
if [[ "$CURRENT_BRANCH" != "$EXPECTED_BRANCH" ]]; then
  echo -e "${RED}Error: on branch '$CURRENT_BRANCH', expected '$EXPECTED_BRANCH'.${NC}"
  echo -e "${YELLOW}Create it first:  git checkout -b $EXPECTED_BRANCH${NC}"
  exit 1
fi
...
echo -e "${GREEN}Ralph loop: BUILD mode — PRD-142 Wave 3 Primitive Hardening (12 agent-safe stories; S3/S14 + S12 deletes are human)${NC}"
...
while true; do
  ...
  if [[ "$RESULT_MSG" =~ RALPH_COMPLETE ]]; then
    ...
    echo -e "${GREEN}PRD-142 Wave 3 agent scope done. Human follow-through: W3-S3 (FE tile), W3-S12 deletes + FE-repoint, W3-S14 (prod migration).${NC}"
    break
  fi
