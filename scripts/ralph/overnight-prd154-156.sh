...
# Overnight Ralph chain — PRD-154 → 155 → 156 (Remediation Chain, Night 1)
#
# Topology: each PRD gets its own worktree + branch, cut from the PREVIOUS
# PRD's final tip (154 cuts from main). Stories commit only behind green
# gates, so a branch tip is always stackable even if a PRD ends BLOCKED.
# 154 = Wave-0 quick wins; 155 = route-contract net; 156 = security/tenancy.
#
...
#
# Usage:
#   ./scripts/ralph/overnight-prd154-156.sh             # full chain, resumes where it left off
#   ./scripts/ralph/overnight-prd154-156.sh 154         # one PRD (validate the kit before stacking)
#   ./scripts/ralph/overnight-prd154-156.sh 155 156     # subset, in the given order
#
# Billing: inherits the environment — subscription by default; export
#   ANTHROPIC_API_KEY before launch to bill the API (predictable per-token burn).
#
# Env knobs:
#   RALPH_MODEL=claude-opus-4-8  model for build+review (default; override e.g. claude-sonnet-4-6)
#   RALPH_MAX_ITERS=25       per-PRD build-iteration cap
#   RALPH_STOP_AT=07:30      wind down gracefully at this wall-clock time
#   RALPH_SKIP_DOCKER_CHECK=1  skip the dev-stack pre-flight (set for 154-156: no compose boot gate)
#
...
REPORT="$RALPH_DIR/night-report-$(date +%F).md"
mkdir -p "$STATE_DIR" "$LOG_DIR"

MAX_ITERS="${RALPH_MAX_ITERS:-25}"
RALPH_MODEL="${RALPH_MODEL:-claude-opus-4-8}"   # headless build+review model (Opus 4.8)
ITER_TIMEOUT="45m"
REVIEW_TIMEOUT="30m"
MAX_REVIEW_CYCLES=2
...
branch_for() {
  case "$1" in
    154) echo "ralph/prd-154-wave0-quick-wins" ;;
    155) echo "ralph/prd-155-route-contract" ;;
    156) echo "ralph/prd-156-security-tenancy" ;;
  esac
}
...
base_for() {
  case "$1" in
    154) echo "main" ;;
    155) echo "$(branch_for 154)" ;;
    156) echo "$(branch_for 155)" ;;
  esac
}
...
open_draft_pr() {
  ...
  (cd "$wt" && gh pr create --draft --base "$base" --head "$br" \
    ...
    --body "Overnight Ralph chain output. Build COMPLETE, acceptance PASS, review PASS. See \`scripts/ralph/night-report-$(date +%F).md\` and per-story commits (AC evidence in bodies). Stacked base: \`$base\` — merge in chain order.") \
    >/dev/null 2>&1 || true
}
...
# --- main --------------------------------------------------------------------------
PRD_LIST=("$@")
[[ ${#PRD_LIST[@]} -eq 0 ]] && PRD_LIST=(154 155 156)
...
rep ""
rep "---"
rep "Chain finished $(date '+%F %H:%M'). Morning protocol: review + merge IN ORDER (154 first, then 155, then 156); each later branch then rebases trivially. Test one worktree at a time."
