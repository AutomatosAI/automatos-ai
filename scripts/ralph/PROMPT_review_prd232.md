...
7. **Gap/decay plumbing (US-011).** The VECTOR replay fixture must produce a gap row through the REAL recorder path (not a hand-inserted row). Decay must floor at seeded confidence, never zero-out the bootstrap. Per-call DB sessions in any new signal path = CRITICAL (the PRD-141 US-019 contract). **EFFECTS-NOT-NAMES (live-test lesson, 2026-08-29):** the gap detector must judge from telemetry rows carrying RESOLVED inner action names + status — never by counting the wrapper name (`platform_execute`) or regex-matching result text. Adversarial test to demand: a turn that creates three agents THROUGH the dispatcher must register ZERO gaps; if the detector would score it as 'no platform calls', that is a false-positive gap factory = HIGH.
...
- **No CRITICAL/HIGH/MEDIUM** → reply exactly `REVIEW_PASS` + a 5-line summary noting: (a) merge path — #643 is CLOSED (superseded by #641 on main) and US-001+US-002 are ALREADY on main via hotfix #654: the supervisor rebases this branch onto main (the two cherry-picked stories fold out), retargets the PR to main, resolves #647's tool_router drift, then merges; (b) post-merge human steps: seed script with --yes, uplift eval run, ≥5pt gate, THEN TOOL_ROUTING_GRAPH=true in Railway; (c) rollback lines (both flags).
