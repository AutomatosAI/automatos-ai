...
# Ralph Wiggum Build Loop (Claude) — Chat Mode Bar
# Usage:
...
PROMPT_FILE="scripts/ralph/PROMPT_build_prd141.md"
...
echo -e "${GREEN}Ralph loop: BUILD mode — PRD-141: Widget Vertical-Agnostic Refactor (~14 executable stories autonomously)${NC}"
...
while true; do
  ...
  if [[ "$RESULT_MSG" =~ RALPH_COMPLETE ]]; then
    ...
    echo -e "${GREEN}PRD-141: all executable tasks finished. Cross-repo (US-013/014/015/016/017), fixtures (US-004), and canary (US-020) remain BLOCKED for human.${NC}"
    break
  fi
  ...
  if [[ "$RESULT_MSG" =~ RALPH_BLOCKED ]]; then
    ...
    echo -e "${YELLOW}Hit a BLOCKED story (cross-repo, requires production fixtures, or operational). Loop halted — see commit log for details.${NC}"
    break
  fi
