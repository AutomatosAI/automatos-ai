...
**CONTEXT — read this twice (CORRECTED 2026-08-27).** PR #613 squash-merged the July branch at **US-008** — US-009/US-010 were pushed after the merge and NEVER reached main. On main and real: `services/onboarding_state.py`, `services/trial_ledger.py`, `platform_update_onboarding` + the intake tools, credential truth, capability report. Build ON those; never re-create, fork, or shadow them. NOT on main: the v2 onboarding section (main's `modules/context/sections/onboarding.py` is the PRD-123-era v1), `tests/test_prd222_trust_guards.py`, and the summary-Deliverable handoff — **US-017/US-018 re-land them via cherry-pick from the old branch** (60a11c936, 82421cb5f); once landed, never weaken the trust-guard test. The PRD's §3 "current reality" predates all of this — trust main + this paragraph, not §3.
